package com.backend.seqaq.tools.fenci.keyword;
 ...
 * @date Oct 20, 2018 tfidf算法原理参考：http://www.cnblogs.com/ywl925/p/3275878.html
...
public class TFIDFAnalyzer {
  ...
  public List<Keyword> analyze(String content, int topN) {
    ...
    for (String word : tfMap.keySet()) {
      ...
      } else keywordList.add(new Keyword(word, idfMedian * tfMap.get(word)));
    }
  ...
  /**
   * tf值计算公式 tf=N(i,j)/(sum(N(k,j) for all k))
  ...
  private Map<String, Double> getTF(String content) {
    ...
    if (content == null || content.equals("")) return tfMap;
  ...
  /**
   * 默认jieba分词的停词表 url:https://github.com/yanyiwu/nodejieba/blob/master/dict/stop_words.utf8
   *
   * @param set
   * @param filePath
   */
  ...
  public static void main(String[] args) {
    ...
    for (Keyword word : list) System.out.print(word.getName() + ":" + word.getTfidfvalue() + ",");
  }
}
